...
class orderFrame(customtkinter.CTkFrame):
  def __init__(self, master, frame_color, main_color, prev_frame):
    ...
    self.summary_frame = customtkinter.CTkFrame(self, width = 300, height = 500, corner_radius = 16, fg_color = 'white')
    ...
    self.total_price_label = customtkinter.CTkLabel(self.summary_frame, text="Total Price: ₱0", font=('Helvetica', 16, 'bold'), text_color='black')
    self.total_price_label.place(x=20, y=150)

    self.get_selected_car = customtkinter.CTkLabel(self.summary_frame, text = '', font = ('Helvetica', 16, 'bold'), text_color = 'black')
    self.get_selected_car.place(x = 50, y = 50)

    self.get_car_price = customtkinter.CTkLabel(self.summary_frame, text='', font = ('Helvetica', 16, 'bold'), text_color = 'black')
    self.get_car_price.place(x = 50, y = 80)
  ...
  def calculate_total_price(self):
    try:
      ...
      if rental_duration > 7:
        self.total_price_label.configure(text="Invalid Date: Duration is capped at 7 days")
        return 
      
      if rental_duration <= 0:
        self.total_price_label.configure(text="Invalid Dates: Duration must be positive")
        return
